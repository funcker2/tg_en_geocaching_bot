...
_S: dict[str, dict[str, str]] = {
    "ru": {
        ...
        # accuracy / live location
        "accuracy_footer": "📡 Точность: ±{acc} м",
        "accuracy_poor_hint": (
            "📡 Точность геолокации сейчас низкая (±{acc} м) — телефон ещё "
            "«прогревает» GPS. Постойте пару секунд на открытом месте, "
            "точность обычно улучшается."
        ),
        "live_location_tip": (
            "💡 Совет: чтобы не отправлять геолокацию вручную каждый раз, "
            "нажмите 📎 (скрепка) → Геопозиция → <b>Транслировать геопозицию</b>. "
            "Тогда расстояние до точек будет обновляться само, пока вы идёте."
        ),
        "location_stale": "⚠️ Ваши координаты устарели. Поделитесь геолокацией ещё раз.",
    },
    ...
    "bg": {
        ...
        # accuracy / live location
        "accuracy_footer": "📡 Точност: ±{acc} м",
        "accuracy_poor_hint": (
            "📡 Точността на локацията в момента е ниска (±{acc} м) — телефонът "
            "все още „загрява” GPS. Постойте няколко секунди на открито място, "
            "точността обикновено се подобрява."
        ),
        "live_location_tip": (
            "💡 Съвет: за да не изпращате местоположението ръчно всеки път, "
            "натиснете 📎 → Локация → <b>Споделяне на местоположение в реално време</b>. "
            "Тогава разстоянието до точките ще се обновява автоматично, докато вървите."
        ),
        "location_stale": "⚠️ Местоположението ви е остаряло. Споделете го отново.",
    },
}
